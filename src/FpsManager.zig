...
pub fn init(mode: FPSMode) Self {
    ...
    self.last_tick_count = sdl.SDL_GetPerformanceCounter();
...
pub fn tick(self: *Self) void {
    ...
    if (self.ticks_per_frame > 0 and elapsed < self.ticks_per_frame) {
        ...
        const wait_ns = @as(f64, @floatFromInt(rem_ticks)) * 1_000_000_000.0 / @as(f64, @floatFromInt(sdl.SDL_GetPerformanceFrequency()));
        // sdl.SDL_DelayNanosecondsPrecise(@as(u64, @intFromFloat(wait_ns)));
        sdl.SDL_DelayNS(@as(u32, @intFromFloat(wait_ns)));
    }

    // Record delta for this frame
    const then = sdl.SDL_GetPerformanceCounter();
    ...
    if (fps_dt >= sdl.SDL_GetPerformanceFrequency()) {
        const fc_f = @as(f64, @floatFromInt(self.frame_counter));
        const freq_f = @as(f64, @floatFromInt(sdl.SDL_GetPerformanceFrequency()));
...
pub fn getDelta(self: Self) f32 {
    ...
    return @as(f32, @floatFromInt(self.last_delta_ticks)) / @as(f32, @floatFromInt(sdl.SDL_GetPerformanceFrequency()));
}
